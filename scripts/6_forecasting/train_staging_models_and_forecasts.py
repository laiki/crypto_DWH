...
def summarize_staging_time_window(staging_db_path: Path) -> tuple[str | None, str | None]:
    ...
    max_epoch = row[1]
    min_utc = (
        datetime.fromtimestamp(int(min_epoch), tz=timezone.utc).isoformat(timespec="seconds")
        if min_epoch is not None
        else None
    )
    max_utc = (
        datetime.fromtimestamp(int(max_epoch), tz=timezone.utc).isoformat(timespec="seconds")
