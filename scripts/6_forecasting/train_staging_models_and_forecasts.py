...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--staging-glob",
        default="data/staging/staging_export_*_last_*.db",
        help="Glob for staging DB files used as training history.",
    )
    parser.add_argument(
        "--cleansing-db",
...
def discover_staging_dbs(staging_glob: str) -> list[Path]:
    paths = sorted(Path(p) for p in glob.glob(staging_glob))
    return [p for p in paths if p.is_file() and p.suffix.lower() == ".db"]
...
def load_staging_series_before_cutoff(
    ...
    *,
    ...
    sql = f"""
        ...
        FROM market_ticks
        WHERE lower(exchange_id) = ?
          AND symbol = ?
          AND ingestion_ts_utc < ?
          AND {price_expr} IS NOT NULL
        ORDER BY ingestion_ts_utc ASC
    """

    for db_path in staging_db_paths:
        ...
        try:
            ...
            if table_exists is None:
                continue
            frame = pd.read_sql_query(sql, connection, params=(exchange_id.lower(), symbol, cutoff_utc))
            if frame.empty:
                continue
            frames.append(frame)
        finally:
...
def summarize_staging_time_window(staging_db_paths: list[Path]) -> tuple[str | None, str | None]:
    global_min_utc: str | None = None
    global_max_utc: str | None = None
    for db_path in staging_db_paths:
        connection = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True)
        try:
            table_exists = connection.execute(
            ...
            if table_exists is None:
                continue
            row = connection.execute("SELECT MIN(ingestion_ts_utc), MAX(ingestion_ts_utc) FROM market_ticks").fetchone()
            if row is None:
                continue
            min_utc = str(row[0]) if row[0] is not None else None
            max_utc = str(row[1]) if row[1] is not None else None
            if min_utc is not None and (global_min_utc is None or min_utc < global_min_utc):
                global_min_utc = min_utc
            if max_utc is not None and (global_max_utc is None or max_utc > global_max_utc):
                global_max_utc = max_utc
        finally:
            connection.close()
    return global_min_utc, global_max_utc
...
def main() -> None:
    ...
    if not staging_db_paths:
        raise SystemExit(f"No staging DBs found for --staging-glob: {args.staging_glob}")
    ...
    with sqlite3.connect(str(cleansing_db_path)) as cleansing_conn:
        ...
        if staging_min_utc is None:
            raise SystemExit("No staging timestamps found in market_ticks. Cannot train forecasting models.")
        ...
        if pd.isna(cutoff_ts) or pd.isna(staging_min_ts):
            ...
            raise SystemExit(
                ...
                "Provide older staging exports (for example with --start-relative-from-hour), "
                "or point --staging-glob to older ingestion DB slices."
            )
